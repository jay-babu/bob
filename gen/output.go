...
func generateComponentFactoryModelsFacade[T, C, I any](
	outFolder string,
	packagePath string,
	modelSplit *ModelSplitData,
	factorySplit *ModelSplitData,
	...
	originalTables := data.Tables
	originalSplit := data.ModelSplit
	originalGeneration := factorySplit.Generation
	originalComponent := factorySplit.CurrentComponent
	defer func() {
		data.Tables = originalTables
		data.ModelSplit = originalSplit
		factorySplit.Generation = originalGeneration
		factorySplit.CurrentComponent = originalComponent
	}()

	data.Tables = filterTablesForComponent(tables, component)
	data.ModelSplit = factorySplit
	factorySplit.Generation = modelSplitGenerationComponent
	factorySplit.CurrentComponent = component

	componentFolder := filepath.Join(outFolder, filepath.FromSlash(component.RelativePath))
	componentPackage := path.Join(packagePath, component.RelativePath)
	if err := generateFactoryModelsFacade(componentFolder, modelSplit, data.FactoryTables(), data); err != nil {
		return "", err
	}
...
func generateFactoryModelsFacade[T, C, I any](
	...
	components := make(map[string]struct{}, len(tables))
