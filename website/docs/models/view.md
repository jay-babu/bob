...
The `NewViewx()` function takes an extra type parameter to determine how slices of the corresponding table struct are returned, and requires a `scan.Mapper` used to scan rows for every query built from the view. Use `NewView()` when reflection-based `scan.StructMapper` behavior is desired.
